...
public class PencilTool extends Filter {
    private List<double[][]> strokesWo;
    private List<double[][]> strokesBres;
    ...
    private boolean reverseBW = false;
    private int gradGap = 1;
    private int binParam = 40;
    private int maxGrayLevel = 90;
    private int strokeStep = 2;
    ...
    public BufferedImage apply(BufferedImage image) {
        ...
        imageToReturn = binImage(imageToReturn, binParam, 128);
    ...
    private BufferedImage applyStrokes(BufferedImage image, int strokesStep) {
        if (strokesWo == null || strokesBres == null) return null;
        ...
        for (int strokeN = 0; strokeN < strokesBres.size(); strokeN++) {
//        for (int strokeN = 2; strokeN < 3; strokeN++) {
            KernelFilter kernelFilter = new KernelFilter(strokesWo.get(strokeN), strokeLength);
            ...
            for (int x = 0; x < imageToReturn.getWidth(); x++) {
                for (int y = 0; y < imageToReturn.getHeight(); y++) {
                    ...
                    if (comp > maxGrayLevel) comp = maxGrayLevel;
    ...
    private List<BufferedImage> generateStrokeMaps(BufferedImage image, int strokesStep) {
        if (strokesBres == null || strokesBres.isEmpty()) return null;

        BufferedImage extendedImage = getExtendedImage(image, strokeLength / 2);
        ArrayList<BufferedImage> C = new ArrayList<>();
        int responseParam = strokeLength * 40;
        for (int i = 0; i < strokesBres.size(); i++) {
        ...
        for (int i = 0; i < xCoords.size() - 1; i++) {
            for (int j = 0; j < yCoords.size() - 1; j++) {
                StrokeMapGenerateThread thread = new StrokeMapGenerateThread(extendedImage, xCoords.get(i), yCoords.get(j),
                        xCoords.get(i + 1), yCoords.get(j + 1), C, strokesBres, responseParam, strokeLength, strokesStep);
                threads.add(thread);
                thread.start();
            }
        }
    ...
    private List<double[][]> generateStrokesWo() {
    ...
    private List<double[][]> generateStrokesBres() {
        List<double[][]> strokes = new ArrayList<>();
        double angle = 22.5;
        int x1 = strokeLength / 2, x2 = strokeLength / 2, y1 = 0, y2 = strokeLength - 1;
        for (int i = 0; i < 3; i++) {
...
//            float grad = (float) dy / dx;
    ...
    public boolean isReverseBW() {
        return reverseBW;
    }

    public void setReverseBW(boolean reverseBW) {
        this.reverseBW = reverseBW;
    }
}
